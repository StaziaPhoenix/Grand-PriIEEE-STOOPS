...
int main(int argc, char** argv) {
  ...
  if (argc == 1) {
    cout << "Supply one or more commands to send to the Arduino\n";
    exit(1);
  }
 ...
 // printf("I2C: acquiring buss to 0x%x\n", ADDRESS);
  ...
  if (ioctl(file, I2C_SLAVE, ADDRESS) < 0) {
    cerr << "I2C: Failed to acquire bus acess/talk to slave t=0x" << ADDRESS << endl;
  ...
  for (arg = 1; arg < argc; arg++) {
    ...
    if (0 == sscanf(argv[arg], "%d", &val)) {
      cerr << "Invalid parameter " << arg <<" \"" << argv[arg] << "\"" << endl; 
